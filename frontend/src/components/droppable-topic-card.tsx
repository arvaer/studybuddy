...
import { ChevronDown, FolderOpen, Folder, Plus, Trash2, FileText, Video, BookOpen, FileIcon, GraduationCap, Sparkles, Loader2 } from "lucide-react";
...
interface DroppableTopicCardProps {
  ...
  onAddConcept?: (topicId: string, name: string) => Promise<void>;
}
              ...
              {showAddInput ? (
                <div className="flex gap-2">
                  <Input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === "Enter" && newConceptName.trim() && !adding) {
                        setAdding(true);
                        onAddConcept?.(topic.id, newConceptName.trim()).finally(() => {
                          setAdding(false);
                          setNewConceptName("");
                  ...
                  <Button
                    size="sm"
                    className="h-8"
                    disabled={!newConceptName.trim() || adding}
                    onClick={() => {
                      if (!newConceptName.trim() || adding) return;
                      setAdding(true);
                      onAddConcept?.(topic.id, newConceptName.trim()).finally(() => {
                        setAdding(false);
                        setNewConceptName("");
