...
function toRu(dto: RuDTO): ReinforcementUnit {
  return {
    ...
    claim: dto.claim,
    context: dto.context,
    claimId: dto.claimId ?? undefined,
    dependencyCost: dto.dependencyCost,
    ...
    sourceResourceId: dto.sourceResourceId ?? undefined,
  };
}
...
export async function createNote(data: {
  ...
  const dto = await apiPost<NoteDTO>("/api/notes", data);
  return toNote(dto);
}
...
export async function createConcept(data: {
  ...
  const dto = await apiPost<ConceptDTO>("/api/concepts", data);
  return toConcept(dto);
}
...
export async function uploadResource(
  ...
  const dto: ResourceDTO = await res.json();
  return toResource(dto);
}

// ─── Create RUs ─────────────────────────────────────────────────────────────

export async function createRUs(
  conceptId: string,
  items: { claim: string; context: string }[],
  sourceResourceId?: string,
): Promise<ReinforcementUnit[]> {
  const dtos = await apiPost<RuDTO[]>("/api/reinforcement-units", {
    conceptId,
    items,
    sourceResourceId,
  });
...
export async function fetchResourceContent(id: string): Promise<string> {
  const data = await apiGet<{ text: string | null }>(`/api/resources/${id}/content`);
  return data.text ?? "";
}
...
export async function fetchResourcePages(
  ...
  const data = await apiGet<{ pages: string[] }>(
    `/api/resources/${id}/pages?start=${start}&end=${end}`,
  );
  return data.pages;
}
